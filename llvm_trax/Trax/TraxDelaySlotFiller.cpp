//===-- DelaySlotFiller.cpp - Trax delay slot filler --------------------===//
//
...
namespace llvm {
cl::opt<bool> DisableDelaySlotFillerTrax(
  "disable-trax-delay-filler",
  cl::init(false),
  cl::desc("Disable the Trax delay slot filter."),
  cl::Hidden);
}
...
static bool hasUnknownSideEffects(MachineBasicBlock::iterator &I) {
  ...
  if (op == Trax::ADDK || op == Trax::ADDIK ||
      op == Trax::ADDC || op == Trax::ADDIC ||
      op == Trax::ADDKC || op == Trax::ADDIKC ||
      op == Trax::RSUBK || op == Trax::RSUBIK ||
      op == Trax::RSUBC || op == Trax::RSUBIC ||
      op == Trax::RSUBKC || op == Trax::RSUBIKC)
    return false;
...
bool Filler::runOnMachineBasicBlock(MachineBasicBlock &MBB) {
  ...
  for (MachineBasicBlock::iterator I = MBB.begin(); I != MBB.end(); ++I)
    if (I->getDesc().hasDelaySlot()) {
      ...
      if (D == MBB.end())
        BuildMI(MBB, ++J, I->getDebugLoc(), TII->get(Trax::NOP));
      else
        MBB.splice(++J, &MBB, D);
    }
  return Changed;
...
/// createTraxDelaySlotFillerPass - Returns a pass that fills in delay
/// slots in Trax MachineFunctions
FunctionPass *llvm::createTraxDelaySlotFillerPass(TraxTargetMachine &tm) {
