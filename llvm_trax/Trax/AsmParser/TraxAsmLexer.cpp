//===-- TraxAsmLexer.cpp - Tokenize Trax assembly to AsmTokens --------===//
//
...
namespace {
  ...
  class TraxBaseAsmLexer : public TargetAsmLexer {
  ...
  public:
    TraxBaseAsmLexer(const Target &T, const MCAsmInfo &MAI)
      : TargetAsmLexer(T), AsmInfo(MAI) {
    }
  };
  
  class TraxAsmLexer : public TraxBaseAsmLexer {
  public:
    TraxAsmLexer(const Target &T, const MCAsmInfo &MAI)
      : TraxBaseAsmLexer(T, MAI) {
      std::string tripleString("trax-unknown-unknown");
...
AsmToken TraxBaseAsmLexer::LexTokenUAL() {
...
extern "C" void LLVMInitializeTraxAsmLexer() {
  RegisterAsmLexer<TraxAsmLexer> X(TheTraxTarget);
}
