//===-- TraxMachineFunctionInfo.h - Private data ----------------*- C++ -*-=//
//
...
//===----------------------------------------------------------------------===//
//
// This file declares the Trax specific subclass of MachineFunctionInfo.
//
//===----------------------------------------------------------------------===//

#ifndef TRAX_MACHINE_FUNCTION_INFO_H
#define TRAX_MACHINE_FUNCTION_INFO_H
...
/// TraxFunctionInfo - This class is derived from MachineFunction private
/// Trax target-specific information for each MachineFunction.
class TraxFunctionInfo : public MachineFunctionInfo {
...
private:
  ...
  /// TraxFIHolder - Holds a FrameIndex and it's Stack Pointer Offset
  struct TraxFIHolder {

    int FI;
    int SPOffset;

    TraxFIHolder(int FrameIndex, int StackPointerOffset)
  ...
  TraxFIHolder GPHolder;

  /// On LowerFormalArguments the stack size is unknown, so the Stack
  /// Pointer Offset calculation of "not in register arguments" must be
  /// postponed to emitPrologue.
  SmallVector<TraxFIHolder, 16> FnLoadArgs;
  bool HasLoadArgs;
  ...
  // postponed to emitPrologue.
  SmallVector<TraxFIHolder, 4> FnStoreVarArgs;
...
public:
  TraxFunctionInfo(MachineFunction& MF)
  ...
  void recordLoadArgsFI(int FI, int SPOffset) {
    ...
    FnLoadArgs.push_back(TraxFIHolder(FI, SPOffset));
  }

  void recordStoreVarArgsFI(int FI, int SPOffset) {
    if (!HasStoreVarArgs) HasStoreVarArgs=true;
    FnStoreVarArgs.push_back(TraxFIHolder(FI, SPOffset));
  }
...
#endif // TRAX_MACHINE_FUNCTION_INFO_H
