...
int calc_dram_channel(long long int physical_address)
{
...
/*
  Once the completion time of a read is known, this function informs the TRaX thread and caches and corrects the "infinite" latency that was assumed
*/
void updateTraxRequest(ThreadState* thread, int which_reg, long long int completion_time, unsigned result, L1Cache* L1, L2Cache* L2, int trax_addr)
{
  ...
  L1->UpdateCache(trax_addr, completion_time);
  L1->UpdateBus(trax_addr, completion_time);
  L2->UpdateCache(trax_addr, completion_time);
}
...
int read_matches_write_or_read_queue(const dram_address_t &physical_address, request_t*& existing_request)
{
...
request_t * insert_read(const dram_address_t &dram_address, int trax_address, long long int arrival_time, int thread_id, int instruction_id, long long int instruction_pc, 
			int which_reg, reg_value result, Instruction::Opcode op, ThreadState* thread, L1Cache* L1, L2Cache* L2)
{
  ...
  //int trax_addr = physical_address;
  ...
  if(read_matches_write_or_read_queue(dram_address, existing_req))
    {
      // Reading to the same cache line as an existing request, just add the new thread's info to it
      // so that we can wake up all threads waiting for that line
      if(existing_req != NULL)
	{
	  // This request's completion time is already known
	  if(existing_req->request_served == 1)
	    {
	      // Hopefully this will never happen
	      if(arrival_time > existing_req->completion_time)
		{
		  printf("error: existing served request not cleaned out yet\n");
		  exit(1);
		}

	      // Return the request to the L2, 
	      // which will see it has already been served, and update the caches
	      //printf("returning read with known future latency\n");
	      return existing_req;

	    }

	  // Otherwise, the request's completion time is still unknown
	  ...
	  trax_request reqToSameLine;
	  reqToSameLine.which_reg = which_reg;
	  reqToSameLine.trax_addr = trax_address;
	  reqToSameLine.thread = thread;
	  reqToSameLine.L1 = L1;
	  reqToSameLine.L2 = L2;
	  reqToSameLine.result = result;
	  existing_req->trax_reqs.push_back(reqToSameLine);
	  return existing_req;
	}
    }
...
request_t * insert_write(const dram_address_t &dram_address, int trax_address, long long int arrival_time, int thread_id, int instruction_id, long long int instruction_pc,
			 int which_reg, reg_value result, Instruction::Opcode op, ThreadState* thread, L1Cache* L1, L2Cache* L2)
{
	  ...
	  // opened row
		...
		case COL_READ_CMD :
		  ...
		  current_col_reads[channel][rank][bank]++;
			...
			//printf("completing %u reads\n", request->trax_reqs.size());
			//printf("dram clock: %lld\n", CYCLE_VAL);
			for(size_t i=0; i < request->trax_reqs.size(); i++)
			  {



			    ThreadState* thread = request->trax_reqs[i].thread;

			    updateTraxRequest(thread, 
					      request->trax_reqs[i].which_reg,
					      (request->completion_time / DRAM_CLOCK_MULTIPLIER),
					      request->trax_reqs[i].result.udata, 
					      request->trax_reqs[i].L1, 
					      request->trax_reqs[i].L2, 
					      request->trax_reqs[i].trax_addr);
			    

			    //printf("\t%u: thread id: %d, which_reg: %d, result: %u, addr: %d, completion time: %lld\n", i, thread->thread_id, 
			    //	   request->trax_reqs[i].which_reg, request->trax_reqs[i].result.udata, request->trax_reqs[i].trax_addr, request->completion_time);
			    
			    /*

			    thread->register_ready[request->trax_reqs[i].which_reg] = request->completion_time;
			    ...
			    // Will need a vector of thread, which_reg, and udata in order to update multiple threads requesting an address on the same line
			    thread->UpdateWriteCycle(request->trax_reqs[i].which_reg, UNKNOWN_LATENCY, request->trax_reqs[i].result.udata, request->completion_time);
			   
			    // TODO: This is potentially wasteful since many of the requests may be coming from the same caches
			    // Use some kind of structure that only keeps track of the unique caches
			    request->trax_reqs[i].L1->UpdateCache(request->trax_reqs[i].trax_addr, request->completion_time);
			    request->trax_reqs[i].L2->UpdateCache(request->trax_reqs[i].trax_addr, request->completion_time);
			    */
