...
// for synchronization
boost::mutex atominc_mutex;
boost::mutex memory_mutex;
boost::mutex sync_mutex;
boost::mutex global_mutex;
boost::mutex profile_mutex;
boost::mutex usimm_mutex[MAX_NUM_CHANNELS];
boost::condition_variable sync_cond;
...
void printUsage(char* program_name) {
  ...
  printf("    --serial-execution     [use a single thread to run simulation]\n");
...
int main(int argc, char* argv[]) {
  ...
  if(serial_execution)
    ...
    threads.reserve(total_simulation_threads);
    for(int i = 0; i < total_simulation_threads; ++i) {
      printf("Creating thread %d...\n", (int)i);
      boost::thread* toAdd = new boost::thread(CoreThread, &args[i]);
