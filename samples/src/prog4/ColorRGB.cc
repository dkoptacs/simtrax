...
// load a color from memory - r, g and b are in consecutive locations
// starting at addr
ColorRGB loadColorFromMem(const int& addr) {
   return ColorRGB(loadf(addr,0), loadf(addr,1), loadf(addr, 2)); 
}
