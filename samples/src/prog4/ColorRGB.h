...
// Prototypes for non-member functions defined inColor3D.cpp
ColorRGB operator* (const float a, const ColorRGB& c);    // multiply by a float on the left
ColorRGB loadColorFromMem(const int& addr);                // load a color from memory
