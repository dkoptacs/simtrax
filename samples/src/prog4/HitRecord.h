...
public: 
  ...
  // set private data of the hit record
  void setTmin(const float& t_in) {Tmin = t_in;}; // Tmin component
  void reset() {       // reset the HitRecord to defaults
    Tmin = FLOAT_MAX;  // set Tmin to infinity(ish)
    Hitp = false;      // reset the hit flag
  } // ObjAddr doesn't need resetting because you only access it on a hit 
