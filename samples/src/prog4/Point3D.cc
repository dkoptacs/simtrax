...
// add a point and a Normal
Point3D Point3D::operator+ (const Normal& n) const {
...
// add normal to the current Point3D
Point3D& Point3D::operator+= (const Normal& n){     
  x+=n.x; y+=n.y; z+=n.z;
  return *this;
}
...
// subtract vector from the current Point3D
Point3D& Point3D::operator-= (const Vector3D& v){     
  x-=v.x; y-=v.y; z-=v.z;
  return *this;
}
...
// subtract normal from the current Point3D
Point3D& Point3D::operator-= (const Normal& n){     
  x-=n.x; y-=n.y; z-=n.z;
  return *this;
}
