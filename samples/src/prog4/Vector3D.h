...
public: 
  ...
  Vector3D operator* (const Vector3D& v);       // component-wise multiply
  
  Vector3D operator/ (const float a) const;     // divide by a float
  Vector3D& operator/= (const float a);		// div current Vector3D by a float
  Vector3D operator/ (const Vector3D& v) const; // div vector by another vector
  ...
  float normalize();		    // normalize this Vector3D (return old length)
  Vector3D& hat();	            // normalize the vector and return it
  Vector3D inverse();               // return Vector3D(1/x, 1/y, 1/z)
...
Vector3D loadVecFromMem(const int& addr);                // load a vector from memory
Vector3D MinComps(const Vector3D& v1, const Vector3D& v2);    // return vector of smallest components
Vector3D MaxComps(const Vector3D& v1, const Vector3D& v2);    // return vector of largest components
