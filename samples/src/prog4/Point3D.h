...
public: 
  ...
  Point3D operator+ (const Normal& n) const;	// add a point and a Normal
  Point3D& operator+= (const Vector3D& v);      // add vector to the current Point3D
  Point3D& operator+= (const Normal& n);        // add Normal to the current Point3D
  
  Vector3D operator- (const Point3D& p) const; // subtract two Point3Ds - get a Vector3D!
  Point3D operator- () const;		       // negate the Point3D
  Point3D& operator-= (const Vector3D& v);     // subtract Vector from point
  Point3D& operator-= (const Normal& n);       // subtract Normal from point
...
Point3D loadPtFromMem(const int& addr);                // load a point from memory
Point3D MinComps(const Point3D& p1, const Point3D& p2);    // return point with smaller components
Point3D MaxComps(const Point3D& p1, const Point3D& p2);    // return point with larger components
