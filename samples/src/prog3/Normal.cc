...
float Normal::normalize() {
  float inv_len = invsqrt(x*x + y*y + z*z); // 1/len (using TRaX intrinsic)
  float len = 1.f/inv_len;                  // len 
...
Normal& Normal::hat(){
  float inv_len = invsqrt(x*x + y*y + z*z); // Get 1/length of vector
  x*=inv_len; y*=inv_len; z*=inv_len; // multiply vec components by 1/length
